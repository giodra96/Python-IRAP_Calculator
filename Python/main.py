...
def creazione_comuni():
    lista_comuni = []
    comunisegnati_visti = set()
    lista_imprese = test_2.lista_impreseordinate()
...
def main ():
    lista_comuni = creazione_comuni() #parto dai comuni già ppresenti nel file txt
    lista_imprese = test_2.lista_impreseordinate()
    ...
    while (stop == False):
        ...
        match value:
            case "1":
                lista_comuni = aggiungi_comune(lista_comuni)
            case "2":
                output = registra_impresa(lista_comuni, lista_imprese)
                lista_comuni = output["comuni"]
                lista_imprese = output["imprese"]
            case "3":
                print(calcola_irap(lista_imprese))
            case "4":
                emissione_modellof24(lista_comuni, lista_imprese)
            case "7":
                stop = True
    for impresa in lista_imprese:
